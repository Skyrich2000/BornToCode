...
{
	while (len--)
		draw_point(pos[0] + dir[0] * len -g()->view.view_xview, pos[1] + dir[1] * len -g()->view.view_yview, color);
}
...
void	draw_debug()
{
	...
	while (++i < OBJ_SIZE)
	{
		...
		while (node)
		{
			...
			draw_bold_point(ins->x - ins->spr->offset_x, ins->y - ins->spr->offset_x, C_YELLOW);
		}
	}
}
