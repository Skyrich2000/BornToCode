...
int A_to_B(int len)
{
	...
	if (len == 2)
	{
		if (st()->n[A][head]->link[next]->data > st()->n[A][head]->link[next]->link[next]->data)
		{
			s(A);
		}
		return (OK);
	...
	//printf("[%d %d] ra %d rb %d pb %d | ", pivot[0], pivot[1], count[ra], count[rb], count[pb]);
	//print_list("A to B after");
...
int B_to_A(int len)
{
	...
	if (len == 2)
	{
		if (st()->n[B][head]->link[next]->data < st()->n[B][head]->link[next]->link[next]->data)
		{
			s(B);
		}
		p(A);
	...
	//printf("[%d %d] ra %d rb %d pa %d | ", pivot[0], pivot[1], count[ra], count[rb], count[pa]);
	//print_list("B to A after");
