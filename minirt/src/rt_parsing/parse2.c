...
{
	if ((mini->scr.wid && mini->scr.hei) || ft_arrsize(line) != 3
...
int	parse_sp(char **line, t_minirt *mini)
{
	...
	if (ft_arrsize(line) != 4 || !ft_atod(&r, line[2])
...
int	parse_sq(char **line, t_minirt *mini)
{
	...
	if (ft_arrsize(line) != 5 || !ft_atod(&side, line[3])
		|| !parse_split(&c, line[1]) || !parse_split(&n, line[2])
...
int	parse_tr(char **line, t_minirt *mini)
{
	...
	if (ft_arrsize(line) != 5 || !parse_split(&p1, line[1])
		|| !parse_split(&p2, line[2]) || !parse_split(&p3, line[3])
		|| !parse_split(&color, line[4]))
		return (ERROR);
	if (!add_world(mini->wrd,
